...
import type { DeckFormField, DeckFormState } from "../../new/actions";
...
export async function updateDeckAction(deckId: string, _previousState: DeckFormState, formData: FormData): Promise<DeckFormState> {
  ...
  if (!user) {
    return createError("덱을 수정하려면 로그인해야 합니다.");
  }

  if (!UUID_PATTERN.test(deckId)) {
    return createError("잘못된 덱입니다.");
  }
  ...
  if (!deck) {
    return createError("존재하지 않는 덱입니다.");
  }

  if (deck.authorId !== user.id) {
    return createError("덱 작성자만 덱을 수정할 수 있습니다.");
  }
  ...
  try {
    ...
    return createError("덱을 수정하지 못했습니다. 잠시 후 다시 시도해주세요.");
  }

  revalidatePath("/decks");
  revalidatePath(`/decks/${deck.id}`);
  revalidatePath(`/decks/${deck.id}/edit`);
